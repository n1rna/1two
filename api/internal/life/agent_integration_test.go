...
func loadTestLLMConfig(t *testing.T) *ai.LLMConfig {
	...
	if os.Getenv("RUN_LIFE_INTEGRATION") != "1" {
		t.Skip("set RUN_LIFE_INTEGRATION=1 to run life agent integration tests")
	}
...
// runAgent runs a single chat turn with mock tools at low temperature for stability.
// It mirrors Agent.buildToolAgentConfig but takes a custom executor and skips DB.
func runAgent(t *testing.T, mock *mockTools, req ChatRequest) *ai.ToolAgentResult {
	t.Helper()
	...
	agentCfg := ai.ToolAgentConfig{
		...
		MaxRounds:   5,
		Temperature: 1.0, // kimi-k2.5 enforces temperature=1.0; matches prod
		MaxTokens:   16384,
		LLMConfig:   cfg,
	}
...
func defaultProfile() *Profile {
	return &Profile{Timezone: "America/Los_Angeles", WakeTime: "07:00", SleepTime: "23:00"}
}

// mockEventsJSON serializes a slice of GCalEvent into the JSON shape the
// get_calendar_events tool returns to the agent.
func mockEventsJSON(events []GCalEvent) string {
...
func TestAgent_RemindMeToCallMom(t *testing.T) {
	mock := newMockTools()
	res := runAgent(t, mock, ChatRequest{
		UserID:      "test-user",
...
func TestAgent_RememberPreference(t *testing.T) {
	mock := newMockTools()
	res := runAgent(t, mock, ChatRequest{
		UserID:      "test-user",
...
func TestAgent_ForgetExistingMemory(t *testing.T) {
	mock := newMockTools()
	memID := "mem_coffee_42"
	res := runAgent(t, mock, ChatRequest{
		UserID:  "test-user",
...
func TestAgent_CreateRoutine(t *testing.T) {
	mock := newMockTools()
	res := runAgent(t, mock, ChatRequest{
		UserID:      "test-user",
...
func TestAgent_LogWeight(t *testing.T) {
	mock := newMockTools()
	res := runAgent(t, mock, ChatRequest{
		UserID:               "test-user",
...
func TestAgent_ListThenDelete(t *testing.T) {
	mock := newMockTools()
	mock.setResponse("list_routines",
		`{"routines":[{"id":"rt_meditate_1","name":"Morning meditation","type":"morning_routine","description":"10 min meditation"}]}`)

	res := runAgent(t, mock, ChatRequest{
		UserID:      "test-user",
		Message:     "Delete my morning meditation routine.",
		Profile:     defaultProfile(),
		AutoApprove: true,
	})
	if !hasTool(res, "list_routines") {
		t.Errorf("expected list_routines before delete; got %v", toolNames(res))
	}
...
func TestAgent_NoActionChitchat(t *testing.T) {
	mock := newMockTools()
	res := runAgent(t, mock, ChatRequest{
		UserID:      "test-user",
...
func TestAgent_ReportsActionsInText(t *testing.T) {
	mock := newMockTools()
	res := runAgent(t, mock, ChatRequest{
		UserID:      "test-user",
...
func TestAgent_MultiAction_WeightAndRoutineUpdate(t *testing.T) {
	mock := newMockTools()
	mock.setResponse("list_routines",
	...
	res := runAgent(t, mock, ChatRequest{
		...
		ConversationCategory: "health",
		HealthProfile: &HealthProfile{
			WeightKg: 78, GoalWeightKg: 74, HeightCm: 178, Age: 32, Gender: "male",
		},
		AutoApprove: true,
	})
	...
	if !hasTool(res, "update_routine") {
		t.Errorf("expected update_routine; got %v", toolNames(res))
	}
	if c := mock.findCall("update_routine"); c != nil {
		if id, _ := c.Args["routine_id"].(string); id != "rt_gym_1" {
			t.Errorf("expected update_routine target rt_gym_1, got %v", c.Args)
		}
	}
}
...
func TestAgent_GetCalendarEvents(t *testing.T) {
	mock := newMockTools()
...
func TestAgent_RescheduleCalendarEvent(t *testing.T) {
	mock := newMockTools()
	now := time.Now().UTC()
	events := []GCalEvent{
		{
...
func TestAgent_DeleteCalendarEvent(t *testing.T) {
	mock := newMockTools()
	now := time.Now().UTC()
	events := []GCalEvent{
		{
...
func TestAgent_LinkEventToRoutine(t *testing.T) {
	mock := newMockTools()
	now := time.Now().UTC()
	events := []GCalEvent{
		{
...
func TestAgent_ListTasks(t *testing.T) {
	mock := newMockTools()
	res := runAgent(t, mock, ChatRequest{
		UserID:      "test-user",
...
func TestAgent_CompleteTask(t *testing.T) {
	mock := newMockTools()
...
func TestAgent_UpdateTask(t *testing.T) {
	mock := newMockTools()
	mock.setResponse("list_tasks",
...
func TestAgent_DeleteTask(t *testing.T) {
	mock := newMockTools()
	mock.setResponse("list_tasks",
		`{"tasks":[{"id":"task_milk_42","title":"Buy milk","status":"needsAction"}]}`)
...
func TestAgent_CreateTaskList(t *testing.T) {
	mock := newMockTools()
	res := runAgent(t, mock, ChatRequest{
		UserID:      "test-user",
...
// ─── health: profile, meal plan, sessions ────────────────────────────────────

func TestAgent_UpdateHealthProfile(t *testing.T) {
	mock := newMockTools()
	res := runAgent(t, mock, ChatRequest{
		UserID:               "test-user",
...
func TestAgent_GenerateMealPlan(t *testing.T) {
	mock := newMockTools()
	res := runAgent(t, mock, ChatRequest{
		UserID:               "test-user",
...
func TestAgent_CreateWorkoutSession(t *testing.T) {
	mock := newMockTools()
	res := runAgent(t, mock, ChatRequest{
		UserID:               "test-user",
	...
	// Verify the exercises actually mention squat / lunge / leg press
...
func TestAgent_AddExerciseToSession(t *testing.T) {
	mock := newMockTools()
	res := runAgent(t, mock, ChatRequest{
		UserID:               "test-user",
...
func TestAgent_UpdateWorkoutSession(t *testing.T) {
	mock := newMockTools()
	res := runAgent(t, mock, ChatRequest{
		UserID:               "test-user",
		...
		HealthProfile: &HealthProfile{
			WeightKg: 80, HeightCm: 180, Age: 30, Gender: "male",
			FitnessLevel: "advanced", FitnessGoal: "hypertrophy",
		},
		ActiveSessions: []SessionSummary{
			{
				ID: "sess_arms_7", Title: "Arms day", Status: "active",
...
// ─── summaries / cross-domain fetch ──────────────────────────────────────────

func TestAgent_GetHealthSummary(t *testing.T) {
	mock := newMockTools()
...
func TestAgent_GetLifeSummary(t *testing.T) {
	mock := newMockTools()
	// Realistic shape: aggregated data, not just counts. Matches what
	// toolGetLifeSummary returns in production.
	...
	// Aggregation rule: after get_life_summary, agent must NOT redundantly
	// drill into the underlying fetchers.
...
func TestAgent_ListActionables(t *testing.T) {
	mock := newMockTools()
...
// ─── ambiguity → actionable (asks user instead of guessing) ──────────────────

func TestAgent_AmbiguousRequest_CreatesActionable(t *testing.T) {
	mock := newMockTools()
	// "Schedule a workout sometime this week" — ambiguous time → agent should
	// ask the user via a choose/input actionable rather than blindly creating one.
	res := runAgent(t, mock, ChatRequest{
		UserID:               "test-user",
		...
		HealthProfile: &HealthProfile{
			WeightKg: 80, HeightCm: 180, Age: 30, Gender: "male",
			FitnessLevel: "intermediate", FitnessGoal: "general_fitness",
		},
		AutoApprove: false, // critical: user wants to be asked
	})
	// Either: agent creates an actionable OR creates a draft session/event.
	// We accept any of: create_actionable, create_calendar_event, create_session.
